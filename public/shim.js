...
Buffer.prototype.copy = function (buffer, t_start, s_start, s_end) {
  t_start = t_start || 0;
  s_start = s_start || 0;
...
Buffer.prototype.slice = function (start, end) {
  ...
  var newb = new Buffer(this.buffer.slice(start, end));
  newb.endian = this.endian;
  return newb;
}

Buffer.prototype.toString = function (type, from, to) {
  switch (type) {
    case 'ascii':
...
(function (Buffer) {
  ...
  keys.forEach(function (key) {
    Buffer.prototype['read' + key.replace('int', 'Int')] = function (offset) {
      return this.dataview['get' + key](offset, this.endian)
    }
    Buffer.prototype['write' + key.replace('int', 'Int')] = function (value, offset) {
